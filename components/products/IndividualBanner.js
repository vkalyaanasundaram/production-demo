...
export default function IndividualBanner({ data }) {
  const MobileBannerImage = data?.mobileBanner?.sourceUrl;
        ...
        <div className={bgWrap}>
          ...
          <div className={bgText}>
            <div className="xs:grid col-auto lg:grid grid-cols-2 gap-1 p-3">
              <div className="text-blue-900 mb-10 z-50">
                <div className="sm:w-full text-5xl xs:w-full text-5xl md:text-5xl">
                  {BannerTitle}
                </div>
                <div className="xs:text-xl mt-10 mb-10 lg:text-2xl text-green-900">
                  {ReactHtmlParser(BannerDescription)}
                </div>

                <div className="xs:text-sm mt-5 mb-5 md:text-xl text-blue-900">
                  {BannerList?.map((value, key) => (
                    <div key={key}>
                      <div className="my-2">{value?.bannerTitle}</div>
                      <div className="text-base leading-8">
                        {ReactHtmlParser(value?.lists)}
                      </div>
                    </div>
                  ))}
